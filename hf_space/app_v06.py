...
def zerogpu_healthcheck() -> str:
    """Keep the Space valid on ZeroGPU; Blender itself remains a CPU subprocess."""
...
Blender dirender sebagai proses **CPU**. ZeroGPU hanya dipakai untuk healthcheck ringan agar Space tetap valid tanpa menghabiskan kuota GPU untuk render yang tidak memerlukannya.
"""
    )
    ...
    with gr.Row():
        mode_input = gr.Radio(
            ["2D Cutout Blender"], value="2D Cutout Blender", label="Mode produksi"
        )
    ...
    with gr.Row():
        duration_input = gr.Slider(3, 8, value=3, step=1, label="Durasi preview")
    ...
    audio_input = gr.Checkbox(
        value=False, label="Metadata audio/dialog saja (audio belum dirender)"
    )
    ...
    with gr.Accordion("File produksi dan bukti Tahap 1–7", open=False):
        json_output = gr.Code(label="animation_job.json", language="json", lines=18)
        with gr.Row():
    ...
    demo.load(
        fn=zerogpu_healthcheck,
        inputs=None,
        outputs=health_output,
        api_name="zerogpu_healthcheck",
    )
